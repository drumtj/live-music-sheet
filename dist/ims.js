...
function MusicSheet(){
  ...
  function create(data, options){
    ...
    let sync = createSync({scroll:options.scroll});
    sync.setContext(ctx);
  ...
  function scrollToStave(staveY){
    if(
     (document.rootElement.scrollTop > staveY - scrollOffset || document.rootElement.scrollTop <= staveY - scrollOffset - (window.innerHeight - 500))
    ){
      // console.error("scroll");
      document.rootElement.scrollTo(0, staveY - scrollOffset);
    }
  }
      ...
      Tone.Draw.schedule(()=>{
        ...
        if(!part._stopped){
          ...
          if(isScrolling && note.staveNote.stave && note.staveNote.stave._isStartMeasure && currentStave != note.staveNote.stave){
            scrollToStave(note.staveNote.stave.y);
